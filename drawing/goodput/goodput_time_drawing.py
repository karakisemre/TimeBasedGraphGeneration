...
# ============================================================
# Core compute (GOODPUT)
# ============================================================
def compute_goodput_series(
    *,
    psnr: PsnrQualityTable,
    collect: CollectLinesTable,
    max_layer: int = 2,
    mode: str = "raw",              # raw | forward | checkpoint | merge
    require_full_start_iframe: bool = False,
    y_unit: str = "Kbps",           # Kbps | Mbps
    min_elapsed_s: float = 1.0,     # initial spike cut
) -> Tuple[List[float], List[float]]:
    """
    Returns (time_s, avg_goodput_[Kbps|Mbps]) as cumulative-average rate.

    mode="raw":
        Goodput = sum(bits of ALL received layers) / elapsed time
        (No GOP validity gating; counts whatever arrived)

    mode="forward"/"checkpoint"/"merge":
        Keeps your previous "usable" gating semantics.
    """

    if mode not in ("raw", "forward", "checkpoint", "merge"):
        raise ValueError("mode must be raw|forward|checkpoint|merge")

    received_layers_by_frame: Dict[int, set[int]] = {}
    time_by_frame_layer: Dict[Tuple[int, int], int] = {}

    # IMPORTANT: if duplicates exist, keep EARLIEST time
    for r in collect.collect_rows:
        if r.frame_no is None or r.quality_layer is None or r.time is None:
            continue
        fn = int(r.frame_no)
        ly = int(r.quality_layer)
        if ly < 0 or ly > max_layer:
            continue

        received_layers_by_frame.setdefault(fn, set()).add(ly)
        key = (fn, ly)
        t = int(r.time)
        if key in time_by_frame_layer:
            time_by_frame_layer[key] = min(time_by_frame_layer[key], t)
        else:
            time_by_frame_layer[key] = t
    ...
    # -------------------------
    # RAW GOODPUT: count everything that arrived
    # -------------------------
    if mode == "raw":
        for (fn, ly), t in time_by_frame_layer.items():
            row = psnr.get(fn, ly)
            if row is None:
                continue
    ...
    # -------------------------
    # USABLE: keep your previous gating
    # -------------------------
    else:
        i_frames = _i_frames_from_psnr(psnr)

        # I frames count immediately (prefix)
        for k in i_frames:
        ...
        for idx in range(len(i_frames) - 1):
            ...
            # checkpoint/merge: validity decided by NEXT I (prefix)
            for l in range(0, max_layer + 1):
                if l > eff_next:
                    break
...
def _apply_layout(fig: go.Figure, *, title: str, y_unit: str, y_max: float) -> None:
    ...
    fig.update_yaxes(
        title_text=f"<b>Average Goodput ({y_unit})</b>",
        title_font=dict(size=18),
        tickfont=dict(size=14),
        ticks="outside",
        showline=True,
...
# ============================================================
# Draw: Fixed & Dynamic (GOODPUT)
# ============================================================
def draw_fixed_plot_goodput(
    *,
    psnr: PsnrQualityTable,
    fixed: List[Experiment],
    out_html: Path,
    mode: str = "raw",      # raw goodput by default
    y_unit: str = "Kbps",
    max_layer: int = 2,
    ...
    for proto, lst in by_proto.items():
        ...
        for e in lst:
            ...
            x, y = compute_goodput_series(
                psnr=psnr, collect=collect,
                max_layer=max_layer,
                mode=mode,
                y_unit=y_unit,
                min_elapsed_s=min_elapsed_s,
                require_full_start_iframe=False,
            )
            all_y.extend(y)
    ...
    y_max = _robust_max(all_y, q=0.995) * 1.10 if all_y else 1.0
    _apply_layout(fig, title=f"Goodput vs Time · Fixed (1/2/3MB) | Mode={mode}", y_unit=y_unit, y_max=y_max)

    out_html.parent.mkdir(parents=True, exist_ok=True)
    fig.write_html(str(out_html), include_plotlyjs="cdn")


def draw_dynamic_plot_goodput(
    *,
    psnr: PsnrQualityTable,
    ...
    for e, k in dyn_filtered:
        ...
        name = base if dup[key] == 1 else f"{base} #{dup[key]}"
    ...
    y_max = _robust_max(all_y, q=0.995) * 1.10 if all_y else 1.0
    _apply_layout(fig, title=f"Goodput vs Time · Dynamic (Scenario1/2) | Mode={mode}", y_unit=y_unit, y_max=y_max)

    out_html.parent.mkdir(parents=True, exist_ok=True)
    fig.write_html(str(out_html), include_plotlyjs="cdn")


def draw_fixed_and_dynamic_goodput(
    *,
    psnr: PsnrQualityTable,
    fixed: List[Experiment],
    dynamic: List[Experiment],
    out_dir: Path,
    mode: str = "raw",   # goodput default
    y_unit: str = "Kbps",
    max_layer: int = 2,
    min_elapsed_s: float = 1.0,
) -> None:
    ...
    draw_fixed_plot_goodput(
        psnr=psnr,
        fixed=fixed,
        out_html=out_dir / f"goodput_fixed_1_2_3_{mode}_{y_unit.lower()}.html",
        mode=mode,
        y_unit=y_unit,
        max_layer=max_layer,
        min_elapsed_s=min_elapsed_s,
    ...
    draw_dynamic_plot_goodput(
        psnr=psnr,
        dynamic=dynamic,
        out_html=out_dir / f"goodput_dynamic_s1_s2_{mode}_{y_unit.lower()}.html",
        mode=mode,
        y_unit=y_unit,
        max_layer=max_layer,
        min_elapsed_s=min_elapsed_s,
